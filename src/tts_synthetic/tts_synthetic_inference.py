...
from tts_synthetic_data import load_stacked_data
...
def run_synthetic_natural(args, llm, output_dir: Path, samples: list):
    """
    Run natural generation on synthetic data.
    Takes sample["arguments"]["gen_args_0"]["arg_0"] as the prompt.
    """
    ...
    # === Generate using the intervened prompt from synthetic data ===
    prompts = [s["full_intervened_prompt"] for s in samples]
    responses = generate_with_two_pass(llm, samples, prompts, think_end_tag)
    save_generations(samples, responses, natural_dir, mode="right")


def run_tts_synthetic_right(args, llm, output_dir: Path, stacks: int) -> int:
    """Run RIGHT mode: extend thinking by stacking 'Wait' interventions."""
    
    _, think_end_tag = get_think_tags(args.model_name, project_root.parent / "configs" / "model_config.yaml")
    
    for stack in range(1, stacks + 1):
        print(f"\n--- RIGHT TTS SYNTHETIC: Stack {stack} ---")
